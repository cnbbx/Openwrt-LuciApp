-- Copyright (C) 2022-2023 cnbbx

module("luci.controller.passwall3", package.seeall)
local api = require "luci.passwall3.api"
local appname = api.appname
local ucic = luci.model.uci.cursor()
local http = require "luci.http"
local util = require "luci.util"
local i18n = require "luci.i18n"
local brook = require("luci.passwall3.brook")
local v2ray = require("luci.passwall3.v2ray")
local xray = require("luci.passwall3.xray")
local hysteria = require("luci.passwall3.hysteria")

function index()
	appname = require "luci.passwall3.api".appname
	entry({"admin", "services", appname}).dependent = true
	entry({"admin", "services", appname, "reset_config"}, call("reset_config")).leaf = true
	entry({"admin", "services", appname, "show"}, call("show_menu")).leaf = true
	entry({"admin", "services", appname, "hide"}, call("hide_menu")).leaf = true
	if not nixio.fs.access("/etc/config/passwall3") then return end
	if nixio.fs.access("/etc/config/passwall3_show") then
		e = entry({"admin", "services", appname}, alias("admin", "services", appname, "settings"), _("passwall 3"), -1)
		e.dependent = true
		e.acl_depends = { "luci-app-passwall3" }
	end
	--[[ Client ]]
...
function reset_config()
	luci.sys.call('/etc/init.d/passwall3 stop')
	luci.sys.call('[ -f "/usr/share/passwall3/0_default_config" ] && cp -f /usr/share/passwall3/0_default_config /etc/config/passwall3')
	luci.http.redirect(api.url())
end

function show_menu()
	luci.sys.call("touch /etc/config/passwall3_show")
...
function hide_menu()
	luci.sys.call("rm -rf /etc/config/passwall3_show")
	luci.sys.call("rm -rf /tmp/luci-*")
	luci.sys.call("/etc/init.d/rpcd restart >/dev/null")
...
function link_add_node()
	...
	luci.sys.call("lua /usr/share/passwall3/subscribe.lua add log")
end
...
function get_now_use_node()
	local e = {}
	local data, code, msg = nixio.fs.readfile("/tmp/etc/passwall3/id/global")
	if data then
...
function get_redir_log()
	...
	if nixio.fs.access("/tmp/etc/passwall3/" .. id .. ".log") then
		local content = luci.sys.exec("cat /tmp/etc/passwall3/" .. id .. ".log")
...
function get_log()
	-- luci.sys.exec("[ -f /tmp/log/passwall3.log ] && sed '1!G;h;$!d' /tmp/log/passwall3.log > /tmp/log/passwall3_show.log")
	luci.http.write(luci.sys.exec("[ -f '/tmp/log/passwall3.log' ] && cat /tmp/log/passwall3.log"))
end

function clear_log()
	luci.sys.call("echo '' > /tmp/log/passwall3.log")
end
...
function urltest_node()
	...
	local e = {}
	e.index = index
	local result = luci.sys.exec(string.format("/usr/share/passwall3/test.sh url_test_node %s %s", id, "urltest_node"))
	local code = tonumber(luci.sys.exec("echo -n '" .. result .. "' | awk -F ':' '{print $1}'") or "0")
	if code ~= 0 then
		local use_time = luci.sys.exec("echo -n '" .. result .. "' | awk -F ':' '{print $2}'")
		if use_time:find("%.") then
...
function set_node()
	...
	ucic:commit(appname)
	luci.sys.call("/etc/init.d/passwall3 restart > /dev/null 2>&1 &")
...
function update_rules()
	...
	luci.sys.call("lua /usr/share/passwall3/rule_update.lua log '" .. update .. "' > /dev/null 2>&1 &")
	http_write_json()
end
...
function server_user_log()
	local id = luci.http.formvalue("id")
	if nixio.fs.access("/tmp/etc/passwall3_server/" .. id .. ".log") then
		local content = luci.sys.exec("cat /tmp/etc/passwall3_server/" .. id .. ".log")
		content = content:gsub("\n", "<br />")
		luci.http.write(content)
	else
		luci.http.write(string.format("<script>alert('%s');window.close();</script>", i18n.translate("Not enabled log")))
...
function server_get_log()
	luci.http.write(luci.sys.exec("[ -f '/tmp/log/passwall3_server.log' ] && cat /tmp/log/passwall3_server.log"))
end

function server_clear_log()
	luci.sys.call("echo '' > /tmp/log/passwall3_server.log")
end

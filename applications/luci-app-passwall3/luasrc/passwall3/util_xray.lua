module("luci.passwall3.util_xray", package.seeall)
...
function gen_config_server(node)
    ...
    if node.outbound_node and node.outbound_node ~= "nil" then
        ...
        if node.outbound_node == "_iface" and node.outbound_node_iface then
            ...
            local outbound_node_t = uci:get_all("passwall3", node.outbound_node)
            ...
            outbound = require("luci.passwall3.util_xray").gen_outbound(nil, outbound_node_t, "outbound")
        end
        if outbound then
...
function gen_config(var)
    ...
    if remote_dns_udp_server or remote_dns_fake then
        ...
        if true then
            local nodes_domain_text = sys.exec('uci show passwall3 | grep ".address=" | cut -d "\'" -f 2 | grep "[a-zA-Z]$" | sort -u')

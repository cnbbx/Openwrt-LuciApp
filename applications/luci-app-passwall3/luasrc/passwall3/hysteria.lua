module("luci.passwall3.hysteria", package.seeall)
...
function to_move(file)
    ...
    local flag = sys.call('pgrep -af "passwall3/.*hysteria" >/dev/null')
    if flag == 0 then
        sys.call("/etc/init.d/passwall3 stop")
    end
    ...
    sys.call("/bin/rm -rf /tmp/hysteria_download.*")
    if flag == 0 then
        sys.call("/etc/init.d/passwall3 restart >/dev/null 2>&1 &")
    end

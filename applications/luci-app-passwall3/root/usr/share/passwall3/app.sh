...
TMP_DNSMASQ_PATH=/tmp/dnsmasq.d/passwall3
...
PROXY_IPV6=0
PROXY_IPV6_UDP=0
LUA_UTIL_PATH=/usr/lib/lua/luci/passwall3
...
lua_api() {
	...
	echo $(lua -e "local api = require 'luci.passwall3.api' print(api.${func})")
}
...
run_global() {
	...
	source $APP_PATH/helper_dnsmasq.sh add TMP_DNSMASQ_PATH=$TMP_DNSMASQ_PATH DNSMASQ_CONF_FILE=/tmp/dnsmasq.d/dnsmasq-passwall3.conf DEFAULT_DNS=$AUTO_DNS LOCAL_DNS=$LOCAL_DNS TUN_DNS=$TUN_DNS
...
start() {
	pgrep -f /tmp/etc/passwall3/bin > /dev/null 2>&1 && {
